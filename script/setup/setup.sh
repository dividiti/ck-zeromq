...
# Skip common setup: should be false for hub/worker.
skip_common_setup=${CK_SKIP_COMMON_SETUP:-""}
echo "- skip common setup: ${skip_common_setup}"
...
# Skip dataset detection: should be false for hub and true for worker.
skip_dataset_detection=${CK_SKIP_DATASET_DETECTION:-""}
echo "- skip dataset detection: ${skip_dataset_detection}"

echo


if [ -z "${skip_common_setup}" ]; then
...
if [ -z "${skip_dataset_detection}" ]; then
  # Detect a preprocessed ImageNet validation dataset (50,000 images).
  echo "Detecting a preprocessed ImageNet validation set ..."
  dataset_dir=${CK_ENV_DATASET_IMAGENET_PREPROCESSED_DIR:-"/datasets/dataset-imagenet-preprocessed-using-opencv-crop.875-full-inter.linear-side.224/ILSVRC2012_val_00000001.rgb8"}
  dataset_tags=${CK_ENV_DATASET_IMAGENET_PREPROCESSED_TAGS:-"preprocessed,using-opencv,universal,crop.875,full,inter.linear,side.224"}
  dataset_version=${CK_ENV_DATASET_IMAGENET_PREPROCESSED_VERSION:-"using-opencv"}
  ck detect soft:dataset.imagenet.preprocessed --full_path=${dataset_dir} --extra_tags=${dataset_tags} --cus.version=${dataset_version} 
